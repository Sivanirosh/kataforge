...
export function parseUserKataImport(input: unknown): UserKataImport {
  return userKataImportSchema.parse(input);
}

export function parseUserKataImportOrPack(
  input: unknown,
): UserKataImport | UserKataPack {
  const packResult = userKataPackSchema.safeParse(input);
  if (packResult.success) return packResult.data;
  return userKataImportSchema.parse(input);
}
